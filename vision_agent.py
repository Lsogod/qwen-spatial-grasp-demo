...
def _get_client():
    ...
    if not api_key:
        raise RuntimeError("Please set MODELSCOPE_API_KEY before using the Qwen pipeline.")
...
def _get_planner_client():
    load_dotenv()
    provider = (
        os.getenv("ACTION_MODEL_PROVIDER")
        or os.getenv("PLANNER_MODEL_PROVIDER")
        or ("qwen" if (os.getenv("MODELSCOPE_API_KEY") or "").strip() else "azure")
    ).strip().lower()

    if provider in {"qwen", "modelscope"}:
        client, model_name = _get_client()
        return "qwen", client, model_name

    if provider in {"azure", "azure_openai"}:
        client, model_name = _get_azure_client()
        return "azure", client, model_name

    raise ValueError("Unsupported ACTION_MODEL_PROVIDER. Use 'qwen' or 'azure'.")
# 基础：读图 & 可视化
...
def generate_first_robot_action(instruction: str, image_url: str) -> str:
    ...
    """
    _, client, deployment = _get_planner_client()
...
def cli_main():
    parser = argparse.ArgumentParser(description="Run the Qwen-first grounding and action planner.")
    parser.add_argument("--img", required=True, help="Input image path or URL.")
    parser.add_argument("--ins", required=True, help="Instruction for the grasp task.")
    parser.add_argument("--save", default="logs/run1", help="Prefix for saved visualizations.")
    parser.add_argument("--context", default="tabletop scene", help="Optional scene context.")
    parser.add_argument("--max-items", type=int, default=1, help="Maximum detections per target part.")
    ...
    res = plan_and_ground_robot_actions(
        ...
        max_items_per_target=args.max_items,
    )
